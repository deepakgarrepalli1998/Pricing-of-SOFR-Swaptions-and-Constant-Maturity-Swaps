# Fixed Income Securities — Swap & Swaption Markets

> **MSc Quantitative Finance | Group Project**  
...
> **Important:** All three notebooks must be kept in the same directory as the Excel file. Parts 2 and 3 dynamically import the preceding notebooks as modules at runtime — no manual copy-pasting of code is needed.
...
**Key outputs:** `T_ois`, `DF_ois`, `fwd_t`, `fwd_r` — consumed by Parts 2 and 3.
...
| **II.2 — SABR Calibration** | Calibrates `(α, ρ, ν)` per slice with `β = 0.75` fixed. Tables of α (vol-of-vol level), ρ (skew), ν (vol-of-vol) |
...
**Key outputs:** `swaption_slices` (with calibrated `sabr_params` per slice), `D_ois`, `D_ts` — consumed by Part 3.
...
| SABR Parameter Interpolation | Builds bilinear interpolators for `(α, ρ, ν)` across arbitrary `(T_exp, T_ten)` pairs — required since CMS pricing needs the smile at non-grid expiries |
...
| IRR Annuity Machinery | Implements the Internal Rate of Return annuity `A_IRR(S)` and its first/second derivatives — core inputs to the convexity integral |
| **III.1a — CMS 10Y Leg** | Values a leg receiving CMS10Y semi-annually over 5 years. Each payment date: computes convexity adjustment via numerical integration over SABR smile |
...
```bash
pip install numpy pandas matplotlib scipy openpyxl nbformat
```

Python 3.9+ recommended. No GPU or external API dependencies.
...
2. Run the notebooks **in order**: Part 1 → Part 2 → Part 3.
